...
SELECT FILE_NO, FILE_RENAME, MEMBER_NICKNAME, 
TO_CHAR(FILE_UPLOAD_DATE, 'YYYY-MM-DD') FILE_UPLOAD_DATE, FILE_PATH
FROM "UPLOAD_FILE" 
...
CHECK("BOARD_DEL_FL" IN ('Y', 'N') ); -- 수행함

-- 댓글 삭제 여부
ALTER TABLE "COMMENT" ADD
CONSTRAINT "COMMENT_DEL_CHECK"
CHECK("COMMENT_DEL_FL" IN ('Y', 'N') ); -- 수행함
...
FROM "COMMENT"
JOIN MEMBER USING(MEMBER_NO)
WHERE BOARD_NO = 1989) C
WHERE COMMENT_DEL_FL = 'N' -- 댓글이 삭제되지 않았거나
OR 0 != (SELECT COUNT(*) FROM "COMMENT" SUB
		WHERE SUB.PARENT_COMMENT_NO = C.COMMENT_NO
		AND COMMENT_DEL_FL = 'N') 
		-- 자식 댓글 중 삭제되지 않은 행이 하나라도 있으면 부모 댓글을 조회 대상에 포함
		-- 현재 댓글 (C.COMMENT_NO)을 부모로 하는 자식 댓글 중 삭제되지 않은 댓글의 행의 개수 카운트
		-- => 삭제되지 않은 댓글이거나, 삭제된 댓글이라도 그 아래에 활성 상태의 자식 댓글이 존재하면 조회되도록 하는 목적의 SQL
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우
		R.OPEN_MEMBER -- NULL인 경우
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
SELECT SUBSTR(PROFILE_IMG, INSTR(PROFILE_IMG, '/', -1) + 1) "rename" FROM "MEMBER"
UNION
